...
#k8s-master2 ansible_host=11.0.1.32 ip=11.0.1.32 ansible_port=22 ansible_user=root
#k8s-master3 ansible_host=11.0.1.33 ip=11.0.1.33 ansible_port=22 ansible_user=root
k8s-node1 ansible_host=11.0.1.34 ip=11.0.1.34 ansible_port=22 ansible_user=root
k8s-node2 ansible_host=11.0.1.35 ip=11.0.1.35 ansible_port=22 ansible_user=root
#etcd1 ansible_host=11.0.1.31 ip=11.0.1.31 ansible_port=22 ansible_user=root
#etcd2 ansible_host=11.0.1.32 ip=11.0.1.32 ansible_port=22 ansible_user=root
#etcd3 ansible_host=11.0.1.33 ip=11.0.1.33 ansible_port=22 ansible_user=root
...
kube_version: '1.28.2'                                                                # kubrenetes 版本信息(ansible中 main.yml 文件引用的)
k8s_version: 'v1.28.2'                                                                # kubrenetes 初始化定义的版本信息
...
Virtual_Router_ID: '51'                                                               # keepalived VRRP 组播协议号(每个局域网环境中唯一)
vip: '11.0.1.31'                                                                      # keepalived vip 地址,如果只是部署单 master 则把 VIP 地址的值写与 master 的 ip 地址相同即可
...
etcd_version: 'v3.5.9'                                                                # ETCD 版本
etcd_conf: "/etc/etcd/"                                                               # ETCD 配置文件路径
etcd_ssl: '/etc/etcd/ssl'                                                             # ETCD 证书存储路径
force_cert_gen: true                                                                 # 是否强制重新生成证书,第一次启动集群需要,如果etcd已经启动了重新跑集群则不需要
force_cert_sync: true                                                                # 是否强制重新分发证书,第一次启动集群需要,如果etcd已经启动了重新跑集群则不需要
etcd_data: '/var/lib/etcd'                                                            # ETCD 数据存储路径
etcd_backup_dir: "/var/lib/etcd-backup"                                               # ETCD 备份路径
etcd_backup_keep_days: 7                                                              # 7 天保留周期
...
[root@k8s-master1 ~]# ansible -i hosts.ini etcd -m copy -a "src=./roles/etcd/files/etcd-v3.5.9-linux-amd64.tar.gz dest=/usr/local/src/etcd-v3.5.9-linux-amd64.tar.gz mode=0644" --become
```
...
nic: 'ens192'                                                                         # keepalived 调用本机网卡
Virtual_Router_ID: '51'                                                               # keepalived VRRP 组播协议号(每个局域网环境中唯一)
vip: '11.0.1.30'                                                                      # keepalived vip 地址,如果只是部署单 master 则把 VIP 地址的值写与 master 的 ip 地址相同即可
api_vip_hosts: apiserver.cluster.local                                                # keepalived vip 地址配置的域名
notification_emails:
  - acassen@firewall.loc                                                              # keepalived 邮箱
  - failover@firewall.loc                                                             # keepalived 邮箱
...
[root@k8s-master1 ~]# ansible -i hosts.ini all -m shell -a "systemctl restart containerd"
